...
#  Last modified by David J Turner (djturner@umbc.edu) 7/24/26, 4:19 PM. Copyright (c) The Contributors.
"""
This module provides functions to generate Chandra images and exposure maps.

It implements XGA wrapper functions to generate photometric (i.e. images, exposure maps, etc.)
data products from Chandra's backend CIAO software.
"""
...
from xga import NUM_CORES, OUTPUT, xga_conf
...
from xga.sources import BaseSource, NullSource

from .run import ciao_call


@ciao_call
def chandra_image_expmap(
    sources: BaseSource | list[BaseSource] | NullSource | BaseSample,
    lo_en: Quantity = Quantity(0.5, "keV"),
    hi_en: Quantity = Quantity(2.0, "keV"),
    num_cores: int = NUM_CORES,
    disable_progress: bool = False,
) -> tuple[list[Any], bool, bool, int, list[Any], list[Any], list[Any], bool]:
    """
    ...
    :return: A tuple with commands, stack, execute, num_cores, product types, paths, metadata, and
        disable_progress flag.
    """
    ...
    if not isinstance(lo_en, Quantity) or not isinstance(hi_en, Quantity):
        raise TypeError(
            "The lo_en and hi_en arguments must be astropy quantities in units that can be converted to keV."
        )
    ...
    for source in sources:
        ...
        if "chandra" not in source.telescopes:
            # Append empty arrays with an explicit dtype
            sources_cmds.append(np.array([], dtype=str))
            sources_paths.append(np.array([], dtype=str))
            sources_extras.append(np.array([], dtype=str))
            sources_types.append(np.empty((0, 3), dtype=str))
            continue

        # Initialize the lists ONLY for sources that actually have Chandra data
        cmds = []
        final_paths = []
        extra_info = []
        ...
        for product in source.get_products("events", telescope="chandra", just_obj=True):
            ...
            # TODO neaten all of this when a get method for events is implemented
            evt_file = product
            # Tell Mypy (static type checker) we know exactly what type get_products returned in this specific case
            if not isinstance(evt_file, BaseProduct):
                raise TypeError(f"Expected evt_file to be an instance of BaseProduct, but got {type(evt_file)}.")
            ...
            # Tell Mypy (static type checker) we know exactly what type get_products returned in this specific case
            if not isinstance(badpix_prod[0], BaseProduct):
                raise TypeError(f"Expected badpix_prod to be a BaseProduct, but got {type(badpix_prod[0])}.")

            if len(badpix_prod) > 1:
                raise ValueError(
                    f"Found multiple bad pixel files for Chandra {obs_id}-{inst}; this should not be "
                    "possible, please contact the developer."
                )
            elif len(badpix_prod) == 0:
                raise ValueError(
                    "No bad pixel has been read in for Chandra {o}-{i}, please check the bad pixel path"
                    " entered in the XGA configuration file - "
                    "{bpf}".format(o=obs_id, i=inst, bpf=xga_conf["CHANDRA_FILES"][f"{inst}_badpix_file"])
                )
            ...
            flux_cmd = (
                ...
                f"xygrid={xygrid_dir}; mv {out_image_file} {final_image_file}; "
                f"mv {out_expmap_file} {final_expmap_file}; "
