...
#  Last modified by David J Turner (turne540@msu.edu) 01/03/2025, 15:38. Copyright (c) The Contributors
...
def ciao_call(ciao_func):
    ...
    def wrapper(*args, **kwargs):
        ...
        cmd_list, to_stack, to_execute, cores, p_type, paths, extra_info, disable = ciao_func(*args, **kwargs)
        ...
        # Any errors raised shouldn't be CIAO, as they are stored within the product object.
        raised_errors = []
        ...
        if to_execute and len(all_run) > 0:
            ...
            # It is possible to call a wrapped CIAO function and find that the products already exist.
            ...
            for product in results[entry]:
                ...
                # In case they are components of an annular spectrum but they are either none or not usable
                    ...
                    warn("An annular spectrum component ({a}) for {o}{i} has not been generated properly, contact "
                         "the development team if a CIAO error is not shown. The std_err entry is:\n\n "
        ...
        # Errors raised here should not be to do with CIAO generation problems, but other purely pythonic errors
        if len(raised_errors) != 0:
            raise Exception(raised_errors)

        # And here are all the errors during CIAO generation, if any
