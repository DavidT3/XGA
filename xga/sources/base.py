...
class BaseSource:
    ...
    def __init__(self, ra: float, dec: float, redshift: float = None, name: str = None,
        ...
        for telescope in SETUP_TELESCOPES:
            ...
            if not os.path.exists(OUTPUT + telescope + "/profiles/{}".format(self.name)):
                os.makedirs(OUTPUT + telescope + "/profiles/{}".format(self.name))

            # And create an inventory file for that directory
            if not os.path.exists(OUTPUT + telescope + "/profiles/{}/inventory.csv".format(self.name)):
                with open(OUTPUT + telescope + "/profiles/{}/inventory.csv".format(self.name), 'w') as inven:
            ...
            if not os.path.exists(OUTPUT + telescope + "/regions/{0}/{0}_custom.reg".format(self.name)):
                os.makedirs(OUTPUT + telescope + "/regions/{}".format(self.name))
                # And a start to the custom file itself, with red (pnt src) as the default colour
                with open(OUTPUT + telescope + "/regions/{0}/{0}_custom.reg".format(self.name), 'w') as reggo:
