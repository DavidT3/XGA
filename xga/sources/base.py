...
from ..utils import ALLOWED_PRODUCTS, XMM_INST, dict_search, xmm_det, xmm_sky, OUTPUT, CENSUS, SRC_REGION_COLOURS, \
    DEFAULT_COSMO, SETUP_TELESCOPES
...
class BaseSource:
    ...
    def __init__(self, ra: float, dec: float, redshift: float = None, name: str = None,
        ...
        # Setting up nested dictionaries to be occupied by telescope top layers
        ...
        self._obs = {}
        self._instruments = {}
        self._blacklisted_obs = {}

        for tscope in self._telescope:
        ...
        if len(sum(self._obs.values(), [])) == 0:
            # DAVID_QUESTION not sure what the len(self.obs is doing) because I thought it was empty
        ...
        # DAVID_QUESTION want to check that at this point you are only using obs ids that have some valid insts etc. 
        self._onaxis = {}
        for tscope in self._obs.keys():
            for o in self._obs[tscope]:
                if not os.path.exists(OUTPUT + tscope + "/" + o):
                    os.mkdir(OUTPUT + tscope + "/" + o)

                if not os.path.exists(OUTPUT + tscope + "/" + '{}/inventory.csv'.format(o)):
                    with open(OUTPUT + tscope + "/" + '{}/inventory.csv'.format(o), 'w') as inven:
            ...
            try:
                on_axis_match = simple_xmm_match(ra, dec, Quantity(5, 'arcmin'))[1][tscope]["ObsID"].values
            except NoMatchFoundError:
                on_axis_match = np.array([])
            self._onaxis[tscope] = list(np.array(self._obs[tscope])[np.isin(self._obs[tscope], on_axis_match)])
