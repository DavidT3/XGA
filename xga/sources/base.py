...
#  Last modified by David J Turner (turne540@msu.edu) 12/10/2023, 22:09. Copyright (c) The Contributors
...
class BaseSource:
    ...
    def __init__(self, ra: float, dec: float, redshift: float = None, name: str = None,
        ...
        if os.path.exists(OUTPUT) and 'XSPEC' in os.listdir(OUTPUT):
            ...
            move('profiles', xmm_dir + 'profiles')
            move('regions', xmm_dir + 'regions')
        ...
        for tel in self.obs_ids:
            ...
            for oi in self.obs_ids[tel]:
                if not os.path.exists(cur_pth + self.obs_ids[tel][oi]):
                    os.makedirs(cur_pth + self.obs_ids[tel][oi])
