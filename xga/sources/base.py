...
#  Last modified by David J Turner (turne540@msu.edu) 26/08/2024, 15:02. Copyright (c) The Contributors
        ...
        if os.path.exists(OUTPUT + "XSPEC/" + self.name) and read_fits:
            ...
            for row_ind, row in cur_fit_inv.iterrows():
                ...
                for oi in list(set(fit_ois)):
                    print(oi)
                    sello = np.argwhere(fit_ois == oi).T
                    print(sello)
                    print(fit_insts[sello])

                oi_dict = {oi: list(fit_insts[np.argwhere(fit_ois == oi).T].astype(str))
                           for oi in list(set(fit_ois))}
