...
#  Last modified by David J Turner (djturner@umbc.edu) 7/27/26, 2:24 PM. Copyright (c) The Contributors.
"""
...
class BaseSource:
    ...
    def __init__(
        ...
        sel_null_obs: list[str] | None = None,
        ...
        self._other_regions: dict | None = None
        self._alt_match_regions: dict | None = None
    ...
    def detected(self) -> dict:
        ...
        if self._detected is None:
            raise ValueError(
                "detected is currently None, BaseSource objects don't have the type "
    ...
    def _existing_xga_products(self, read_fits: bool, load_spectra: bool, load_profiles: bool) -> None:
        ...
        def parse_image_like(file_path: str, exact_type: str, telescope: str, merged: bool = False) -> Image | ExpMap:
            """
        ...
        def parse_spectrum(row: NamedTuple, combined_obs: bool) -> tuple[Spectrum | None, int | None, int | None]:
            """
        ...
        for tel in reg_paths:
            ...
            for obs_id in reg_paths[tel]:
                ...
                if reg_dict[tel][obs_id][0] is not None and len(reg_dict[tel][obs_id]) > 1:
                    ...
                    diff_sort = np.array([_dist_from_source(*self._ra_dec, r) for r in reg_dict[tel][obs_id]]).argsort()
    ...
    def _generate_interloper_mask(self, mask_image: Image, region_distance: Quantity | None = None) -> ndarray:
        ...
        #  regions we want to include.
            ...
            for_mask = (
                Quantity(
                    np.array(
                        [_dist_from_source(*self._ra_dec, r) for r in self._interloper_regions[mask_image.telescope]]
                    )
                )
                < region_distance
            )
    ...
    def _get_phot_prod(
        ...
        if not psf_corr and with_lims:
            ...
            matched_prods = self.get_products(
                prod_type, obs_id, inst, extra_key=energy_key + extra_key, telescope=telescope
            )
            ...
            broad_matches = self.get_products(
                prod_type, obs_id, inst, extra_key=None, just_obj=False, telescope=telescope
            )
    ...
    def _get_prof_prod(
        ...
        if annuli_bound_radii is not None:
            matched_prods = [
                m_prod for m_prod in matched_prods if len(annuli_bound_radii) == len(m_prod.annulus_bounds)
            ]
    ...
    def get_products(
        ...
        telescope: str | None = None,
    ) -> list[BaseProduct] | list[BaseProfile1D]: ...
    ...
    def get_products(
        ...
        telescope: str | None = None,
    ) -> list[list[str | BaseProduct | BaseProfile1D]]: ...
    ...
    def get_images(
        ...
        For combined images (multi-observation or multi-instrument), you can pass 'combined' as the obs_id and/or
        inst parameter to retrieve products generated by merging data.

        :param str obs_id: Optionally, a specific obs_id to search for can be supplied. The default is None,
            which means all images matching the other criteria will be returned. Pass 'combined' to retrieve
            multi-observation combined images.
        :param str inst: Optionally, a specific instrument to search for can be supplied. The default is None,
            which means all images matching the other criteria will be returned. Pass 'combined' to retrieve
            multi-instrument combined images.
    ...
    def get_expmaps(
        ...
        For combined exposure maps, you can pass 'combined' as the obs_id and/or inst parameter.

        :param str obs_id: Optionally, a specific obs_id to search for can be supplied. The default is None,
            which means all exposure maps matching the other criteria will be returned. Pass 'combined' to retrieve
            multi-observation combined exposure maps.
        :param str inst: Optionally, a specific instrument to search for can be supplied. The default is None,
            which means all exposure maps matching the other criteria will be returned. Pass 'combined' to retrieve
    ...
    def get_ratemaps(
        ...
        For combined ratemaps, you can pass 'combined' as the obs_id and/or inst parameter.

        :param str obs_id: Optionally, a specific obs_id to search for can be supplied. The default is None,
            which means all ratemaps matching the other criteria will be returned. Pass 'combined' to retrieve
            multi-observation combined ratemaps.
        :param str inst: Optionally, a specific instrument to search for can be supplied. The default is None,
            which means all ratemaps matching the other criteria will be returned. Pass 'combined' to retrieve
    ...
    def get_combined_images(
        ...
        For eROSITA, the obs_id and inst parameters control which combination type is retrieved:
    ...
    def get_spectra(
        ...
        For combined spectra (multi-observation or multi-instrument), you can pass 'combined' as the obs_id and/or
        inst parameter. This provides explicit control over which combination type to retrieve for eROSITA:
        - obs_id='227093', inst='combined': Single obs + all instruments (Scenario 1)
        - obs_id='combined', inst='tm1': All obs + single instrument (Scenario 2)
        - obs_id='combined', inst='combined': All obs + all instruments (Scenario 3)

        :param str/Quantity outer_radius: The name or value of the outer radius that was used for the generation of
            the spectrum (for instance 'r200' would be acceptable for a GalaxyCluster, or Quantity(1000, 'kpc')). If
            'region' is chosen (to use the regions in region files), then any inner radius will be ignored.
        :param str obs_id: Optionally, a specific obs_id to search for can be supplied. The default is None,
            which means all spectra matching the other criteria will be returned. Pass 'combined' to retrieve
            multi-observation combined spectra.
        :param str inst: Optionally, a specific instrument to search for can be supplied. The default is None,
            which means all spectra matching the other criteria will be returned. Pass 'combined' to retrieve
            multi-instrument combined spectra.
        :param str/Quantity inner_radius: The name or value of the inner radius that was used for the generation of
            the spectrum (for instance 'r500' would be acceptable for a GalaxyCluster, or Quantity(300, 'kpc')). By
            default this is zero arcseconds, resulting in a circular spectrum.
        :param bool group_spec: Was the spectrum you wish to retrieve grouped?
    ...
    def get_combined_spectra(
        ...
        For eROSITA, the inst parameter controls which combination type is retrieved:
        - inst='combined': Multi-obs + multi-inst (all TMs across all observations)
        - inst='tm1' (or specific): Multi-obs + single inst (one TM across all observations)
        - inst=None: All multi-obs spectra (returns both types if they exist)

        For XMM/Chandra: Not currently implemented - raises NotImplementedError.

        :param str/Quantity outer_radius: The name or value of the outer radius that was used for the generation of
            the spectrum (for instance 'r200' would be acceptable for a GalaxyCluster, or Quantity(1000, 'kpc')). If
            'region' is chosen (to use the regions in region files), then any inner radius will be ignored.
        :param str inst: Optionally, a specific instrument to search for can be supplied. The default is None,
            which means all combined spectra matching the other criteria will be returned. Pass 'combined' to retrieve
            multi-instrument combined spectra, or a specific instrument name to retrieve single-instrument combined.
        :param str/Quantity inner_radius: The name or value of the inner radius that was used for the generation of
            the spectrum (for instance 'r500' would be acceptable for a GalaxyCluster, or Quantity(300, 'kpc')). By
            default this is zero arcseconds, resulting in a circular spectrum.
        :param bool group_spec: Was the spectrum you wish to retrieve grouped?
        ...
        :param float over_sample: If the spectrum you wish to retrieve was over sampled, what was the level of
            over sampling used?
        :param str telescope: Optionally, a specific telescope to search for can be supplied. The default is None,
            which means all combined spectra matching the other criteria will be returned.
        :return: An XGA Spectrum object (if there is an exact match), or a list of XGA Spectrum objects (if there
            were multiple matching products).
        :rtype: Union[Spectrum, List[Spectrum]]
        """
    ...
    def get_annular_spectra(
        ...
        if radii is not None:
            ...
            #  and then exact comparisons no longer work
            matched_prods = [
                m_prod for m_prod in matched_prods if np.allclose(radii, m_prod.radii, rtol=0, atol=RAD_MATCH_PRECISION)
            ]
    ...
    def get_profiles(
        ...
        :param str obs_id: Optionally, a specific obs_id to search for can be supplied. The default is None,
            which means all profiles matching the other criteria will be returned. Pass 'combined' to retrieve
            multi-observation combined profiles.
        :param str inst: Optionally, a specific instrument to search for can be supplied. The default is None,
            which means all profiles matching the other criteria will be returned. Pass 'combined' to retrieve
        ...
        matched_prods = self._get_prof_prod(
            search_key,
            obs_id,
            inst,
            central_coord,
    ...
    def get_lightcurves(
        ...
        if telescope is None and pattern == "default" and len(self.telescopes) != 1:
            warn(
                "Can't use the 'default' pattern argument value when 'telescope' is None and there is more than"
                " one telescope associated with the source - this is because different telescopes use different"
                " pattern styles. The 'pattern' argument value has been set to None.",
                stacklevel=2,
            )
            pattern = None
    ...
    def get_combined_lightcurves(
        ...
        :param Quantity time_bin_size: The time bin size used to generate the desired lightcurve. The default value
            is None, in which case all lightcurves matching other criteria will be retrieved.
        :param dict pattern: Event selection patterns used to create lightcurves of interest. The default
            is 'default' which uses the default values for generating lightcurves for different instruments, or you
            can pass a dictionary with patterns in; e.g. {'pn': '<=4', 'mos': '<=12'}. You can also pass None, which
            means all light curves matching other search terms will be returned.
        :param str telescope: Optionally, a specific telescope to search for lightcurves can be supplied. The
            default is None, which means all lightcurves matching the other criteria will be returned.
        :param str inst: The instrument to search for. For multi-instrument combined lightcurves, pass 'combined'.
            Default is 'combined'.
        :return: An XGA LightCurve object (if there is an exact match), or a list of XGA LightCurve objects (if there
            were multiple matching products).
        :rtype: Union[LightCurve, List[LightCurve]]
        ...
        if telescope == "xmm":
            ...
            for lc in some_lcs:
                if isinstance(patt_search, str):
                    ...
                    rel_patt_search = [patt for i, patt in patt_search.items()]
    ...
    def snr_ranking(
        ...
        :param bool stacked_inst: If True, then SNR-ranking will be performed at an ObsID level, using stacked
            instrument data for each ObsID (if available). Default is False.
    ...
    def count_ranking(
        ...
        :param str/List[str] telescope: The telescopes to calculate count rankings for. Default is
            None, meaning all associated telescopes will be used.
        :param bool stacked_inst: If True, then count-ranking will be performed at an ObsID level, using stacked
            instrument data for each ObsID (if available). Default is False.
        :return: Two dictionaries; the first contains ObsID-instrument information (as an Nx2 array) sorted by
        ...
        # These dictionaries will have telescope name as top level keys, and will store the calculated counts
        #  and the ObsID-inst combos
        obs_inst_dict = {}
        cnts_dict = {}
        for tel in telescope:
            try:
                ...
                obs_inst = []
                cnts = []
                # We loop through the ObsIDs associated with this source and the instruments
                #  associated with those ObsIDs
                for obs_id in self.instruments[tel]:
                    # If the stacked instrument argument is NOT true, then this is every instrument available for
                    #  the current ObsID. Otherwise, it's just 'combined' (i.e. the stack of all instruments
                    #  for the current ObsID)
    ...
    def info(self) -> None:
        ...
        if (
            ...
            print(f"Richness - {self._richness.round(2)}±{self._richness_err.round(2)}")
        elif (
            self._richness is not None
            and self._richness_err is not None
            ...
            print(
                f"Richness - {self._richness.round(2)} -{self._richness_err[0].round(2)}"
                f"+{self._richness_err[1].round(2)}"
            )
        elif self._richness is not None and self._richness_err is None:
            print(f"Richness - {self._richness.round(2)}")
