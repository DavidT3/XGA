...
#  Last modified by David J Turner (djturner@umbc.edu) 7/27/26, 1:52 PM. Copyright (c) The Contributors.
"""
...
class ExtendedSource(BaseSource):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            telescope,
            search_distance,
            back_inn_rad_factor=back_inn_rad_factor,
        ...
        #  in each observation.
        self._detected = {
            tel: {o: self._regions[tel][o] is not None for o in self._regions[tel]} for tel in self.telescopes
        }
        ...
        if True in flat_det and False in flat_det:
            warn_text = f"{self.name} has not been detected in all region files."
            if not self._samp_member:
                warn(warn_text, stacklevel=2)
            else:
                self._supp_warn.append(warn_text)
        ...
        #  and if no custom region options are passed by the user then an error is raised.
            ...
            warn_text = (
                f"{self.name} has not been detected in ANY region files, so generating and fitting products"
                " with the 'region' reg_type will not work"
            )
            if not self._samp_member:
                warn(warn_text, stacklevel=2)
            else:
            ...
            raise NoRegionsError(
                f"{self.name} has not been detected in ANY region files, and no custom region or "
                "overdensity radius has been passed. No analysis is possible."
            )
        ...
        if not in_sample:
            ...
            from ..generate.multitelescope.phot import all_telescope_combined_expmaps, all_telescope_combined_images
        ...
        if clean_obs and clean_obs_reg in self._radii:
            ...
            if len(reject_dict) != 0:
                ...
                if regen_merged:
                    from ..generate.multitelescope.phot import (
                        all_telescope_combined_expmaps,
                        all_telescope_combined_images,
                    )
        ...
        # Throws an error if a poor choice of region has been made
            ...
            raise NoRegionsError(f"{clean_obs_reg} is not associated with {self.name}")
    ...
    def point_clusters(self) -> tuple[dict[str, ndarray], dict[str, list[ndarray]]]:
        ...
        :return: A numpy array of the positions of points of the chosen cluster (not 'galaxy cluster',
            ...
            that were found
        :rtype: tuple[dict[str, ndarray], dict[str, list[ndarray]]]
        """
        return self._chosen_peak_cluster, self._other_peak_clusters

    def find_peak(
        self, rt: RateMap, method: str = "hierarchical", num_iter: int = 20, peak_unit: UnitBase = deg
        ...
        while count < num_iter:
            ...
            separation = Quantity(
                np.sqrt(
                    abs(peak_deg[0].value - central_coords.ra.value) ** 2
                    + abs(peak_deg[1].value - central_coords.dec.value) ** 2
                ),
                deg,
            )
    ...
    def get_peaks(self, telescope: str, obs_id: str | None = None, inst: str | None = None) -> Quantity:
        ...
        if telescope not in self.telescopes:
            raise NotAssociatedError(f"Telescope {telescope} is not associated with {self.name}.")
        elif obs_id is not None and obs_id not in self.obs_ids[telescope]:
            raise NotAssociatedError(f"The ObsID {obs_id} is not associated with {self.name}.")
            ...
            raise NotAssociatedError(
                f"The instrument {inst} is not associated with observation {obs_id} of this source."
            )
    ...
    def get_1d_brightness_profile(
        ...
        if obs_id == "combined" or inst == "combined":
            ...
            interim_prods = self.get_profiles(
                "brightness", obs_id, inst, central_coord, radii=radii, lo_en=lo_en, hi_en=hi_en, telescope=telescope
            )
...
class PointSource(BaseSource):
    ...
    def __init__(
        ...
        if self._redshift is not None and point_radius.unit.is_equivalent("kpc"):
            ...
            raise UnitConversionError(f"Can't convert {point_radius.unit} to a XGA supported length unit")
        self._radii["search"] = search_aperture
            ...
            if len(reject_dict) != 0:
                ...
                if len(self._obs) == 0:
                    raise NoValidObservationsError(
                        "Observation cleaning has been run and there are no remaining observations."
                    )
