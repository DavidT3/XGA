...
#  Last modified by David J Turner (david.turner@sussex.ac.uk) 03/02/2021, 17:05. Copyright (c) David J Turner
...
from ..products.profile import ProjectedGasTemperature1D, XSPECNormalisation1D
...
class GalaxyCluster(ExtendedSource):
    ...
    def get_xspec_norm_profiles(self, radii: Quantity = None, link_norm: bool = False,
                                group_spec: bool = True, min_counts: int = 5, min_sn: float = None,
                                over_sample: float = None, set_id: int = None) \
            -> Union[XSPECNormalisation1D, List[XSPECNormalisation1D]]:
        """
        ...
        :param int set_id: The unique identifier of the annular spectrum set used to generate the profile.
            Passing a value for this parameter will override any other information that you have given this method.
        :return: An XGA XSPECNormalisation1D object if there is an exact match, a list of such objects
            if there are multiple matches.
        :rtype: Union[ProjectedGasTemperature1D, List[ProjectedGasTemperature1D]]
        """
        if link_norm:
            matched_prods = self._get_spec_based_profiles("combined_1d_xspec_norm_profile", radii, group_spec,
                                                          min_counts, min_sn, over_sample, set_id)
        else:
            matched_prods = self._get_spec_based_profiles("1d_xspec_norm_profile", radii, group_spec, min_counts,
                                                          min_sn, over_sample, set_id)

        if len(matched_prods) == 1:
            matched_prods = matched_prods[0]
        elif len(matched_prods) == 0:
            raise NoProductAvailableError("No matching XSPEC normalisation profiles can be found.")
