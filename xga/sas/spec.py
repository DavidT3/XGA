...
def spectrum_set(sources: Union[BaseSource, BaseSample], radii: Union[List[Quantity], Quantity],
    ...
    for s_ind, source in enumerate(sources):
        ...
        if len(exists) == 0:
            # If it doesn't exist this makes sure the spectra are generated by evselect, even if matching ones
            #  already exist
            force_gen = True
        else:
            # This means evselect can use its own checking function to notice that the spectra already exist, as
            #  they must do if the combined_spectrum does
            force_gen = False
        ...
        for r_ind in range(len(radii[s_ind])-1):
            ...
            spec_cmd_out = _spec_cmds(source, radii[s_ind][r_ind+1], radii[s_ind][r_ind], group_spec, min_counts,
                                      min_sn, over_sample, one_rmf, num_cores, disable_progress, force_gen)
