...
def luminosity_temperature_pipeline(sample_data: pd.DataFrame, start_aperture: Quantity, use_peak: bool = False,
    ...
    if isinstance(outer_aperture, str):
        if outer_aperture.lower() == rad_temp_rel.outer_aperture.lower() and outer_aperture.lower() == temp_lum_rel.outer_aperture.lower():
            warn("Your choice of the defination of 'outer_aperture' matches the RT/LT ScalingRelation.")
        elif outer_aperture.lower() != rad_temp_rel.outer_apertured.lower():
            raise TypeError("Mismatch: Your choice of the defination of 'outer_aperture' does not match the RT ScalingRelation.")
        elif outer_aperture.lower() != temp_lum_rel.outer_aperture.lower():
