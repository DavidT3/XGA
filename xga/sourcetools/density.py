...
def _dens_setup(sources: Union[GalaxyCluster, ClusterSample], outer_radius: Union[str, Quantity],
    ...
    spectra/fits have been generated/run, then uses them to calculate the conversion factors from 
    count-rate/volume to squared hydrogen number density.
    ...
    :param Quantity/Dict[str, Quantity] conv_temp: If set this will override XGA measured 
        ...
        dictionary of quantites with telescope keys to specifiy telescope specific temperatures. The
        quantity should have an entry for each cluster being analysed. Default is None.
    :param str/Quantity conv_outer_radius: The outer radius within which to generate spectra and 
    ...
    # Perform some checks on the ObsID and instrument parameters to make sure that they are in the 
    # correct format if they have been set. We don't need to check that the ObsIDs are associated 
    # with the sources here, because that will happen when the ratemaps are retrieved from the 
    # source objects.
...
def _run_sb(src: GalaxyCluster, telescope: str, outer_radius: Quantity, use_peak: bool, 
            lo_en: Quantity, hi_en: Quantity, psf_corr: bool, psf_model: str, psf_bins: int, 
            psf_algo: str, psf_iter: int, pix_step: int, min_snr: float, obs_id: str = None, 
            inst: str = None) -> SurfaceBrightness1D:
    """
    An internal function for the Surface Brightness based density functions, which just quickly 
    assembles the requested surface brightness profile.
    ...
    :param bool psf_corr: Default True, whether PSF corrected ratemaps will be used to make the
        surface brightness profile, and thus the density (if False density results could be 
        incorrect).
    ...
    :param str obs_id: The ObsID of the ratemap that should be used to generate the brightness 
        profile, default is None in which case the combined ratemap will be used.
    :param str inst: The instrument of the ratemap that should be used to generate the brightness 
        profile, default is None in which case the combined ratemap will be used.
    ...
    try:
        sb_prof = src.get_1d_brightness_profile(rad, obs_id, inst, centre, lo_en=lo_en, hi_en=hi_en,
                                                pix_step=pix_step, min_snr=min_snr, 
                                                psf_corr=psf_corr, psf_model=psf_model, 
                                                psf_bins=psf_bins, psf_algo=psf_algo,
                                                psf_iter=psf_iter, telescope=telescope)
    except NoProductAvailableError:
        try:
            sb_prof, success = radial_brightness(rt, centre, rad, src.background_radius_factors[0],
                                                src.background_radius_factors[1], int_mask, 
                                                src.redshift, pix_step, kpc, src.cosmo, min_snr)
        except ValueError:
            sb_prof = None
            success = False
...
def inv_abel_fitted_model(sources: Union[GalaxyCluster, ClusterSample], 
    ...
    profile. Then a conversion factor calculated from simulated spectra is used to infer the number 
    density profile.

    Depending on the chosen surface brightness model, the inverse abel transform may be performed 
    using an analytical solution, or numerical methods.

    :param GalaxyCluster/ClusterSample sources: A GalaxyCluster or ClusterSample object to measure 
        density profiles for.
    :param str/List[str]/BaseModel1D/List[BaseModel1D] model: The model(s) to be fit to the cluster 
        surface profile(s). You may pass the string name of a model (for single or multiple 
        clusters), a single instance of an XGA model class (for single or multiple clusters), a list
        of string names (one entry for each cluster being analysed), or a list of XGA model 
        instances (one entry for each cluster being analysed).
    ...
    :param int num_steps: If using mcmc fitting, the number of steps each walker should take. 
        Default is 20000.
    ...
    :param Quantity/Dict[str, Quantity] conv_temp: If set this will override XGA measured 
        temperatures within the conv_outer_radius, and the fakeit run to calculate the normalisation
        conversion factor will use these temperatures. This can be set as a quantity, or a 
        dictionary of quantites with telescope keys to specifiy telescope specific temperatures. 
    ...
    :param str inv_abel_method: The method which should be used for the inverse abel transform of 
        ...
        inverse abel transform, or 'direct' for models which don't have an analytical solution. 
        Default is None.
    :param int num_cores: The number of cores that the evselect call and XSPEC functions are allowed
        to use.
    ...
    #  passed into the fit method of the surface brightness profile(s)
    ...
    with tqdm(desc="Fitting data, inverse Abel transforming, and measuring densities",
        ...
        for src_ind, src in enumerate(sources):
            for tel in src.telescopes:
                ...
                sb_prof.fit(cur_model, fit_method, num_samples, num_steps, num_walkers, 
                            show_warn=show_warn, progress_bar=False)
                ...
                if model_r.success:
                    ...
                    dens_rads_errs = sb_prof.radii_err.copy()
                    dens_deg_rads = sb_prof.deg_radii.copy()
                    # Run the inverse abel transform for the model, to retrieve distributions for 
                    # the value of the transformed model at each r point. If the user hasn't set a 
                    # method then we use the default method for the current model, otherwise we pass 
                    # the user's choice
                    if inv_abel_method is None:
                        transformed = model_r.inverse_abel(dens_rads, use_par_dist=True)
                    else:
                        transformed = model_r.inverse_abel(dens_rads, use_par_dist=True, 
                                                           method=inv_abel_method)
                    ...
                    if sb_prof.values_unit.is_equivalent('ct/(s*arcmin**2)'):
                        ...
                        #  units of ct/s/(arcmin^2 kpc), so I create a quantity which will convert 
                        #  the arcmin^2 to kpc^2
                        conv = Quantity(ang_to_rad(Quantity(1, 'arcmin'), src.redshift, 
                                                   src.cosmo).to("kpc").value, 'kpc/arcmin')**2
                        transformed /= conv
                    elif sb_prof.values_unit.is_equivalent('ct/(s*kpc**2)'):
                        pass
                    else:
                        raise NotImplementedError("Haven't yet added support for surface "
                                                  "brightness profiles in other units, don't "
                                                  "really know how you even got here.")

                    # We convert the volume element to cm^3 now, this is the unit we expect for the
                    # density conversion
                    transformed = transformed.to('ct/(s*cm^3)')

                    # We multiply by the conversion factor that is unique to the cluster and 
                    # calculated earlier to take the transformed profile to a gas number density 
                    # (n_gas as seen in Eckert et al. 2016, eq. 2).
                    ...
                    try:
                        # I now allow the user to decide if they want to generate number or mass 
                        # density profiles using this function, and here is where that distinction 
                        # is made
                        if num_dens:
                            dens_prof = GasDensity3D(dens_rads.to("kpc"), med_num_dens, 
                                                     sb_prof.centre, src.name, cur_obs, cur_inst, 
                                                     model_r.name, sb_prof, dens_rads_errs, 
                                                     num_dens_err, deg_radii=dens_deg_rads, 
                                                     telescope=tel)
                        else:
                    ...
                    #  passed to the GasDensity3D declaration, this is where an error will be thrown
                        ...
                        warn("One or more of the quantities passed to the init of {}'s density " 
                            "profile has a NaN or Inf value in it.".format(src.name))
                else:
                    final_dens_profs[tel].append(None)
