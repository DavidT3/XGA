...
#  Last modified by David J Turner (turne540@msu.edu) 20/02/2023, 16:51. Copyright (c) The Contributors
import os
...
from ..exceptions import NoMatchFoundError, NoValidObservationsError, NoRegionsError
...
def xmm_region_match(src_ra: Union[float, np.ndarray], src_dec: Union[float, np.ndarray],
    ...
    if generate_ims:
        ...
        # I don't super like this way of doing it, but this is where images generated by XGA will be stored, so
        #  we check and remove any ObsID that already has had exposure maps generated for that ObsID. Normally
        #  XGA sources do this automatically, but NullSource is not as clever as that
        ipath = OUTPUT + "{o}/{o}_{i}_0.5-2.0keVimg.fits"
        uniq_obs_ids = [o for o in uniq_obs_ids if not os.path.exists(ipath.format(o=o, i='pn')) and
                        not os.path.exists(ipath.format(o=o, i='mos1')) and
                        not os.path.exists(ipath.format(o=o, i='mos2'))]

        try:
            # Declaring the NullSource with all the ObsIDs that; a) we need to use to check whether coordinates fall on
            #  an XMM camera, and b) don't already have images maps generated
            obs_src = NullSource(uniq_obs_ids)
            # Run image generation for those ObsIDs - not exposure maps because images take less time to generate
            #  and all we want from this is the WCS information
            evselect_image(obs_src, num_cores=num_cores)
        except NoValidObservationsError:
            pass
